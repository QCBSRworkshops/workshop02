...
<br>
<br>
Check the [tidyverse style guide](https://style.tidyverse.org/) for good coding habits.
...
<br>
<br>
Check-out the paper by [Broman & Woo (2017)](https://doi.org/10.1080/00031305.2017.1375989) 
for useful tips on data organization in spreadsheets. 
...
**Long format**
```{r, echo=FALSE, eval = TRUE}
long = pivot_wider(wide, Measurement, Value, DBH, Height)
long
```
]
...
```{r}
tidyr::tibble(x = 1:3, y = c("a","b","c"))
```


In addition, it simplify the use of nested lists. 
Example: 
```{r}
tidyr::tibble(x = 1:3, y = list(1:5, 1:10, 1:20))
...
`pivot_wider()` - convert from wide to long format

`pivot_longer()` - convert from long to wide format
...
`pivot_wider(data, cols, values_to, ...)`

<span style="font-size: 21px;">
  - `data` A data frame (e.g. `wide`)
  - `cols` name of the new column containing variable names (e.g. `Measurement`)
  - `values_to` name of the new column containing variable values (e.g. `Value`)
...
`pivot_longer(data, cols, values_to)`

<span style="font-size: 21px;">
  - `data` A data frame (e.g. `long`)
  - `cols` Name of the column containing variable names (e.g. `Measurement`)
  - `values_to` Name of the column containing variable values (e.g. `Value`)
</span>
...
```{r}
wide2 = pivot_longer(long, Measurement, Value)
wide2
```
...
```{r}
messy.long <- pivot_wider(messy, taxa, count, -id, -trt)
head(messy.long)
```
...
Converting from wide to long format using `pivot_wider()`

Converting from long format to wide format using `pivot_longer()`
...
```{r}
air.long <- pivot_wider(airquality, variable, value, -Month, -Day)
head(air.long)
```
.small[.comment[
Note that the syntax used here indicates that we wish to gather ALL the columns except *Month* and *Day*. It is equivalent to: `pivot_wider(airquality, value, Ozone, Solar.R, Temp, Wind)`
]]
...
```{r}
air.wide <- pivot_longer(air.long, variable, value)
head(air.wide)
```
...
identical(weight_diff_rbase, as.data.frame(weight_diff))
