...
```{r}
messy.long <- pivot_longer(messy,names_to = "taxa",
...
Using the airquality dataset, `pivot_wider` all the columns (except Month and Day) into rows.

```{r}
air.long <- pivot_longer(airquality,
                         cols =  c("Solar.R", "Wind", "Temp"), 
                         names_to = c("variable") )
...
`pivot_wider` the resulting data frame to return to the original data format.
